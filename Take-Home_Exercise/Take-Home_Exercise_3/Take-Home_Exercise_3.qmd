...
gt_table <- model_comparison %>%
  gt() %>%
  tab_header(
    title = "Comparison of Curve Fitting Models"
  ) %>%
  data_color(
    columns = vars(`Overdispersion`),
    colors = scales::col_factor(
      palette = c("No" = "tomato", "Yes" = "lightgreen"),
      domain = c("No", "Yes")
    )
  ) %>%
  data_color(
    columns = vars(Flexibility),
    colors = scales::col_factor(
      palette = c("Low" = "#f4cccc", "Moderate" = "#fce5cd", "High" = "#d9ead3", "Very High" = "#b6d7a8"),
      domain = c("Low", "Moderate", "High", "Very High")
    )
  ) %>%
  data_color(
    columns = vars(`Overfitting Risk`),
    colors = scales::col_factor(
      palette = c("Low" = "#b6d7a8", "Medium" = "#ffe599", "High" = "#f4cccc"),
...
change to 3rd degree and replot
...
![](images/clipboard-2000178618.png){width="651"}

## 6.2. Time Series Curve Fitting

![](images/clipboard-4047672289.png)
