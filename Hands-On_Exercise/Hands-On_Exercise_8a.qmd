...
Among the four packages, **readr**, **tidyr** and **dplyr** are part of **tidyverse** package (only need to load tidyverse).
...
```{r}
mpsz <- st_read(dsn = "Data Files/geospatial", 
...
```{r}
popdata <- read_csv("Data Files/respopagesextod2011to2020.csv")
```
...
#### 2.2. Joining attribute and geospatial data
...
```{r}
write_rds(mpsz_pop2020, "Data Files/mpszpop2020.rds")
```
...
##### 3.3.3. Using *tm_fill()* and *tm_border()*
...
#### 3.4. Plotting choropleth map with built-in classification

##### 3.4.1. Quantile data classification
...
##### 3.4.2. Equal data classification
...
### 4. Drawing small multiple Chloropleth maps
...
#### 4.1. Assigning multiple values to at least one aesthetic argument
...
#### 4.2. Defining group-by variable in *tm_facets()*
...
#### 4.3. Creating multiple stand-alone maps with *tmap_arrange()*
...
### 5. Mapping spatial object that meets selection criterion
...
Reference: <https://r4va.netlify.app/chap21>
